...
class StoryProcessingState(BaseModel):
    __tablename__ = "story_processing_states"
    __table_args__ = (
        Index(
        ...
        CheckConstraint("attempt_count >= 0", name="ck_story_processing_states_attempt_count_nonnegative"),
        CheckConstraint(
            """
            (
                claimed_at IS NULL
            ...
            """,
            name="ck_story_processing_states_claim_complete",
        ),
        CheckConstraint(
            "claim_expires_at IS NULL OR claim_expires_at > claimed_at",
            name="ck_story_processing_states_claim_expiry",
        ),
        CheckConstraint(
            """
            (
            ...
            """,
            name="ck_story_processing_states_processed_identity",
        ),
    )
    ...
    story: Mapped["Story"] = relationship(back_populates="processing_states")
    runs: Mapped[list["StoryProcessingRun"]] = relationship(back_populates="processing_state")


class StoryProcessingRun(Base):
    __tablename__ = "story_processing_runs"
    __table_args__ = (
        Index("uq_story_processing_runs_state_attempt", "processing_state_id", "attempt_number", unique=True),
        CheckConstraint("attempt_number > 0", name="ck_story_processing_runs_attempt_number_positive"),
        CheckConstraint("finished_at IS NULL OR finished_at >= started_at", name="ck_story_processing_runs_finished_after_started"),
        CheckConstraint(
            "outcome IS NULL OR outcome IN ('succeeded', 'failed', 'skipped', 'lease_lost')",
            name="ck_story_processing_runs_outcome",
        ),
        CheckConstraint(
            "(finished_at IS NULL AND outcome IS NULL) OR (finished_at IS NOT NULL AND outcome IS NOT NULL)",
            name="ck_story_processing_runs_completion_pair",
        ),
    )
    ...
    processing_state_id: Mapped[UUID | None] = mapped_column(
        PG_UUID(as_uuid=True),
        ForeignKey("story_processing_states.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    pipeline: Mapped[str] = mapped_column(String(100), nullable=False, index=True)
    ...
    story: Mapped["Story"] = relationship(back_populates="processing_runs")
    processing_state: Mapped["StoryProcessingState | None"] = relationship(back_populates="runs")
