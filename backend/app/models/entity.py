...
class Entity(BaseModel):
    ...
    article_mentions: Mapped[list["ArticleEntity"]] = relationship(
        back_populates="entity",
        cascade="all, delete-orphan",
    )
...
class EntityAlias(BaseModel):
    ...
    __table_args__ = (
        ...
        Index(
            "uq_entity_aliases_active_entity_alias",
            "entity_id",
            "normalized_alias",
            unique=True,
            postgresql_where=text("deleted_at IS NULL"),
    ...
    original_alias: Mapped[str] = mapped_column(
        String(500),
        nullable=False,
    )
...
class ArticleEntity(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "start_offset",
            "end_offset",
            name="uq_article_entity_mention",
            postgresql_nulls_not_distinct=True,
        ),
        CheckConstraint(
            "confidence BETWEEN 0 AND 1",
            name="ck_article_entities_confidence",
        ),
        CheckConstraint(
            "salience BETWEEN 0 AND 1",
            name="ck_article_entities_salience",
        ),
        CheckConstraint(
            "start_offset IS NULL OR start_offset >= 0",
            name="ck_article_entities_start_offset",
        ),
        CheckConstraint(
            "end_offset IS NULL OR end_offset > start_offset",
            name="ck_article_entities_end_offset",
        ),
        CheckConstraint(
            "(start_offset IS NULL) = (end_offset IS NULL)",
            name="ck_article_entities_offsets_pair",
        ),
        CheckConstraint(
            "sentence_index IS NULL OR sentence_index >= 0",
            name="ck_article_entities_sentence_index",
        ),
    )
    ...
    normalized_mention: Mapped[str] = mapped_column(
        String(500),
        nullable=False,
        index=True,
    )
    ...
    text_source: Mapped[TextPart] = mapped_column(
        Enum(TextPart, name="article_text_part"),
        nullable=False,
        index=True,
    )
    ...
    entity: Mapped[Entity] = relationship(
        back_populates="article_mentions",
    )
    article: Mapped["Article"] = relationship(
        back_populates="entity_mentions",
    )
    processing_run: Mapped["ArticleProcessingRun | None"] = relationship()