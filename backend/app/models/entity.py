...
class ArticleEntity(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint("article_id", "entity_id", "text_part", "normalized_mention", "start_offset", "end_offset", name="uq_article_entity_mention", postgresql_nulls_not_distinct=True),
    ...
    entity_type: Mapped[EntityType] = mapped_column(Enum(EntityType, name="entity_type", create_type=False), nullable=False, index=True)
    text_part: Mapped[TextPart] = mapped_column(Enum(TextPart, name="article_text_part"), nullable=False, index=True)
