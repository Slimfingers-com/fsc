...
class Topic(BaseModel):
    ...
    __table_args__ = (
        Index(
            "uq_topics_active_normalized_name",
            "normalized_name",
            unique=True,
            postgresql_where=text("deleted_at IS NULL"),
        ),
        Index(
            "uq_topics_active_slug",
            "slug",
            unique=True,
            postgresql_where=text("deleted_at IS NULL"),
        ),
...
class ArticleTopic(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint(
            "article_id",
            "topic_id",
            name="uq_article_topics_article_topic",
        ),
        CheckConstraint(
            "relevance BETWEEN 0 AND 1",
            name="ck_article_topics_relevance",
        ),
        CheckConstraint(
            "confidence BETWEEN 0 AND 1",
            name="ck_article_topics_confidence",
        ),
    )
