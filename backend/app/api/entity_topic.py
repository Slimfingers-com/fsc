...
def article_entities(article_id: UUID, db: Session = Depends(get_db)):
    rows = db.execute(select(ArticleEntity).options(selectinload(ArticleEntity.entity)).join(Article).join(Feed).join(Source).join(ArticleEntity.entity).where(Article.id == article_id, *_active_article_conditions(), Article.normalized_at.is_not(None), Article.normalized_text.is_not(None), ArticleEntity.deleted_at.is_(None), ArticleEntity.entity.has(deleted_at=None)).order_by(ArticleEntity.entity_id, ArticleEntity.start_offset.nullslast(), ArticleEntity.id)).scalars().all()
...
def article_topics(article_id: UUID, db: Session = Depends(get_db)):
    rows = db.scalars(select(ArticleTopic).options(selectinload(ArticleTopic.topic)).join(Article).join(Feed).join(Source).where(Article.id == article_id, *_active_article_conditions(), Article.normalized_at.is_not(None), Article.normalized_text.is_not(None), ArticleTopic.deleted_at.is_(None), ArticleTopic.topic.has(deleted_at=None)).order_by(ArticleTopic.relevance.desc(), ArticleTopic.topic_id)).all()
    if not rows and not db.scalar(select(Article.id).join(Feed).join(Source).where(Article.id == article_id, *_active_article_conditions())):
        raise HTTPException(404, "Article not found")
...
def _latest(db: Session, relation, id_column, object_id: UUID):
    statement = select(Article).join(relation).join(Feed).join(Source).where(id_column == object_id, relation.deleted_at.is_(None), Article.normalized_at.is_not(None), Article.normalized_text.is_not(None), *_active_article_conditions()).order_by(Article.published_at.desc().nullslast(), Article.id).limit(10)
    articles = list(db.scalars(statement).unique().all())
    count = db.scalar(select(func.count(func.distinct(Article.id))).select_from(Article).join(relation).join(Feed).join(Source).where(id_column == object_id, relation.deleted_at.is_(None), Article.normalized_at.is_not(None), Article.normalized_text.is_not(None), *_active_article_conditions())) or 0
