...
async function request<T>(
  ...
  if (!response.ok) {
    let message = `Backend antwortete mit HTTP ${response.status}`;
    try {
...
export async function searchArticles(options: {
  ...
  set(params, "sort", options.sort ?? "relevance");
  set(params, "page", options.page ?? 1);
  return request<Page<SearchHit>>("/search", params);
}
...
export async function listStories(options: {
  ...
  const params = new URLSearchParams();
  set(params, "sort", options.sort ?? "newest");
  set(params, "page", options.page ?? 1);
  set(params, "min_sources", options.minSources ?? 1);
  return request<Page<StorySummary>>("/stories", params);
}

export function getStory(storyId: string): Promise<StoryDetail> {
  return request<StoryDetail>(`/stories/${encodeURIComponent(storyId)}`);
}
...
export async function getStoryAnalysis(
  ...
  try {
    return await request<StoryAnalysis>(
      `/stories/${encodeURIComponent(storyId)}/analysis`,
    );
  } catch (error) {
